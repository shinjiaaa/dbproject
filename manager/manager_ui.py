...
def show_manager_ui(root):
    ...
    tk.Button(
        ...
        text="도서 추가",
        width=13,
        command=lambda: add_book_ui.show_add_book_ui(root),
    ).pack(pady=2)
    tk.Button(
        frame_left,
        text="도서 삭제",
        width=13,
        command=lambda: delete_book_ui.show_delete_book_ui(root),
    ).pack(pady=2)
    tk.Button(
        frame_left,
        text="반납",
        width=13,
        command=lambda: return_book_ui.show_return_book_ui(root),
    ).pack(pady=2)
    ...
    tk.Label(frame_right, text="블랙리스트 관리", font=("Arial", 13)).pack(
        pady=(20, 10)
    )
    tk.Button(
        frame_right,
        text="블랙리스트 관리",
        width=13,
        command=lambda: blacklist_ui.show_blacklist_ui(
            root, lambda: show_manager_ui(root)
        ),
    ).pack(pady=2)
